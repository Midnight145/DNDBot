    ...
    def create_campaign(self, vals: CampaignInfo) -> bool:
        ...
        try:
            self.bot.db.execute(
                ...
                f"meeting_day, meeting_time, system, new_player_friendly, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, "
                f"?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (vals.name, vals.dm, vals.role, vals.category, vals.information_channel, vals.min_players,
                 ...
                 vals.session_length, vals.meeting_frequency, vals.meeting_day, vals.meeting_time, vals.system,
                 vals.new_player_friendly, vals.timestamp))
            return True
        except Exception:
            traceback.print_exc()
            return False
